...
library BLS {
    ...
    function pubkeyToUncompressed(
        uint256[2] memory compressed,
        uint256[2] memory y
    ) internal pure returns (uint256[4] memory uncompressed) {
        uint256 decision = compressed[0] & SIGN_MASK;
        require(
            decision == ODD_NUM || y[0] & 1 != 1,
            "BLS: bad y coordinate for uncompressed key"
        );
    ...
    function signatureToUncompressed(uint256 compressed, uint256 y)
        internal
        pure
        returns (uint256[2] memory uncompressed)
    {
        uint256 decision = compressed & SIGN_MASK;
        require(
            decision == ODD_NUM || y & 1 != 1,
            "BLS: bad y coordinate for uncompressed key"
        );
